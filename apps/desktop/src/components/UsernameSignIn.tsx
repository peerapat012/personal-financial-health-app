...
export function UsernameSignIn({ onConnected, notice }: { onConnected: (session: SessionResponse) => void; notice: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      clearApiToken();
      setError(failure instanceof ApiError && failure.status === 401 ? "Username or password was not accepted." : failure instanceof ApiError && failure.status === 429 ? "Too many sign-in attempts. Try again in a minute." : failure instanceof ApiError ? failure.message : "Sign-in could not be completed. Try again.");
  ...
  return <main className="session-page">
    ...
    <form className="session-card" onSubmit={submit}><p className="eyebrow">Welcome back</p><h2>Sign in</h2><p className="text-sm text-muted-foreground">Use the username and password provisioned for your account.</p><label htmlFor="username">Username</label><input id="username" name="username" required minLength={3} maxLength={30} pattern="[a-zA-Z0-9_.]+" autoComplete="username" autoFocus value={username} disabled={pending} aria-invalid={Boolean(error)} aria-describedby={error || notice ? "sign-in-error" : undefined} onChange={(event) => setUsername(event.target.value)} /><label htmlFor="password">Password</label><input id="password" name="password" type="password" required minLength={12} maxLength={128} autoComplete="current-password" value={password} disabled={pending} aria-invalid={Boolean(error)} aria-describedby={error || notice ? "sign-in-error" : undefined} onChange={(event) => setPassword(event.target.value)} />{(error || notice) && <p id="sign-in-error" className="form-error" role="alert">{error || notice}</p>}<Button className="session-submit" type="submit" disabled={pending}>{pending ? "Signing in…" : "Sign in"}</Button><p className="field-help">Your password and session are not saved by this app.</p></form>
  </main>;
}
